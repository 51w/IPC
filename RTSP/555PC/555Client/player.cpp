...
#define H264_STREAM_URL "rtsp://%s:8554/main"
void *envThead(void* arg);
void *dec264Thead(void* arg);
...
int init(char* serverIP) 
{
    ...
    char url[256] = { 0 };
    sprintf(url, H264_STREAM_URL, serverIP);
    printf("URL is:%s\n", url);
	...
	pthread_t pt; //
	pthread_create(&pt, NULL, &dec264Thead, 0);
...
void *dec264Thead(void* arg)
{
	...
	while(decflag)
	{
		if(PullBuf(&data) && data.len>0)
		{
		  ...
		  int ddec = DecodeFrame(data.bptr, data.len, &Pyuv);
		  gettimeofday(&Time_end,NULL);

		  int time_use=(Time_end.tv_sec-Time_start.tv_sec)*1000+(Time_end.tv_usec-Time_start.tv_usec)/1000;
		  std::cout << "===" << time_use << "ms===\n";

		  if(ddec == 0)
		  {
			std::cout << "=== " << Pyuv.w << "   " << Pyuv.h << " ===\n";
		  }
		}
	}
}
...
void *envThead(void* arg)
{
	//LOGD("EnvThead:%d", eventLoopWatchVariable);
	env->taskScheduler().doEventLoop(&eventLoopWatchVariable);
	//LOGE("cleanUp null?");
...
int openURL(UsageEnvironment& env, char const* progName, char const* rtspURL) {
  ...
  rtsp_client = ourRTSPClient::createNew(env, rtspURL, RTSP_CLIENT_VERBOSITY_LEVEL, progName);
  if (rtsp_client == NULL) {
...
{
  memcpy(_buff+4, fReceiveBuffer, frameSize);
  
  BufData data;
  data.bptr = _buff;
  data.len  = frameSize+4;
