...
HI_S32 SAMPLE_COMM_VENC_SaveH264(FILE* fpH264File, VENC_STREAM_S* pstStream)
{
    ...
    for (i = 0; i < pstStream->u32PackCount; i++)
    {
		...
		BufData data;
		data.bptr = pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset + 4;
		data.len  = pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset - 4;
		data.naltype = pstStream->pstPack[i].DataType.enH264EType;
		//printf("==========%d\n", data.naltype);
